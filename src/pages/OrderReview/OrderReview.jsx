...
const OrderReview = () => {
  ...
  const handleSubmit = async (productId) => {
    ...
    if (review.rating > 0 && review.rating <= 5 && review.comment) {
      let reviewsApi = clientApi.service("reviews");
      try {
        let response = await reviewsApi.create({
          productID: productId,
      ...
      message.warning(t("Please rate and comment for product"));
    }
  };
      ...
      <div className="order-review-content mt-5">
        ...
        {orderDetail?.orderItems?.length > 0 &&
          orderDetail.orderItems.map((item, index) => {
            const review =
              reviews.find((r) => r.productId === item.product._id) || {};
            return (
                ...
                <div className="info-section">
                  ...
                  <div className="mb-3">
                    ...
                    <Rate
                      className="ms-3"
                      onChange={(value) =>
                        handleRatingChange(value, item.product._id)
                      }
                      value={review.rating}
                    />
                  </div>
                  <Input.TextArea
                    ...
                    onChange={(e) =>
                      handleCommentChange(e, item.product._id)
                    }
                  />
